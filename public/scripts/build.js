...
(function() {
  ...
  function counter(delay, $el, countTo, startNo) {
    var inserter = throttle(function (no) {
      $el.html(addCommas(no));
    }, delay, { trailing: true });
  ...
  function driveSectionAnimation(elToAnimate, elThatTriggersAnimation, animation, animAdaptClass) {
    ...
    var $elThatTriggersAnimation = $(elThatTriggersAnimation);

    animAdaptClass = animAdaptClass || '';

    new Waypoint.Inview({
      element: $(elThatTriggersAnimation)[0],
      entered: function(direction) {
        if ('down' === direction) {
          $el.addClass('animated ' + animation + (' ' + animAdaptClass));
        }
      }
    });
  ...
  driveSectionAnimation('.section3-right-content', '.section13-content img', 'fadeInUp');
...
$(document).ready(function(){
  ...
  var focusClasses = navIds
    ...
    .join(' ');
  function setNavFocus(id) {
    $nav.removeClass(focusClasses);
    $nav.addClass('focus-' + id);
  }
  navIds.forEach(function(navId, index) {
    new Waypoint({
      element: document.getElementById(navId),
      handler: function(direction) {
        if (direction === 'up' && index > 0) {
          setNavFocus(navIds[index - 1]);
        } else {
          setNavFocus(navId);
        }
      },
